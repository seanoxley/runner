(function() {	
	ButtonPadButton = function( parent, name, color ) {
		var that = this;
		this.parent = parent;
		this.name = name;
		this.color = color;
		...
		this.press = function() {
			...
			Game.GameHandler.SoundManager.Sounds.button_pad.sound.play();
		};
		...
		function addEventListeners() {
			that.container.interactive = true;
			...
			function click( event ) {
				var data = Game.GameHandler.ButtonPad.data;
				if ( !data || that.pressed ) {
					return false;
				}
				var dataObject = {};
				dataObject.type = event.type;
				dataObject.button = that.name;
				dataObject.data = data[ that.getIndex() ].data;

				that.press();

				// process click
				Game.GameHandler.DataHandler.processClickData( dataObject );
			}
		}
